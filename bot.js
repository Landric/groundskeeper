...
client.on('guildMemberAdd', member => {
	if(status != "silent"){
		...
		member.guild.channels.get(process.env.ENTRY_CHANNEL).send(message.replace('$name$', '<@'+member.id+'>'));
		//member.guild.channels.get(process.env.ENTRY_CHANNEL).send(grammar.flatten('#welcome#').replace('$name$', '<@'+member.id+'>')); 	
	}
});
